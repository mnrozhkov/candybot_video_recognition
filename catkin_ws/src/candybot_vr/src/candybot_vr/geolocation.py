...
def get_coordinates_by_ip(country_code=8, mobile_network_code=None, radio_type='gsm',
                    provider=None, geoloc_ip='true'):
    '''Returns location coordinates by IP-address
    Args:
    ...
    Returns:
        {'lat':lat,'lng':lng,'accuracy': accuracy}: coordinates and accuracy if data recieved
    ...
    try:
        ...
        return {'lat': json_response['location']['lat'],
                'lng': json_response['location']['lng'],
...
def get_address_info(address):
    '''Returns geografic information by address
    Args:
...
def get_location_by_address(address):
    '''Returns location coordinates by address
    Args:
        address: place address in free string representation
    Returns:
        {'lat':lat,'lng':lng}: coordinates and accuracy if data recieved
        None: if coordinates are not available
    '''
    address_info = get_address_info(address=address)
    if not address_info is None:
        return address_info['results'][0]['geometry']['location']
    else:
        return None
